const GRID_SIZE = 40;
        ...
        var options = {
            ...
            manipulation: {
                ...
                addEdge: function(data, callback) {
                    if (data.from !== data.to) {
                        ...
                        if (weight && !isNaN(weight) && weight.trim() !== "") {
                            data.label = weight.trim();
                            callback(data);
                            showToast(`เชื่อมต่อเส้น ${data.from} ↔ ${data.to} (น้ำหนัก: ${data.label})`, 'success');
                        } else {
                            callback(null);
                        }
                    } else {
                ...
                editEdge: function(data, callback) {
                    var currentWeight = data.label || "1";
                    var weight = prompt("แก้ไขระยะทางใหม่ (ตัวเลข):", currentWeight);
                    if (weight && !isNaN(weight) && weight.trim() !== "") {
                        data.label = weight.trim();
                        callback(data);
                        showToast(`อัปเดตน้ำหนักเส้นเป็น ${data.label}`, 'success');
                    } else {
                        callback(null);
                    }
                },
                deleteNode: function(data, callback) {
                    callback(data);
                    showToast("ลบโหนดที่เลือกแล้ว", "info");
                },
                deleteEdge: function(data, callback) {
                    callback(data);
                    showToast("ลบเส้นที่เลือกแล้ว", "info");
                }
            }
        };
    ...
    function triggerAddEdge() {
        if (!network) return;
        network.addEdgeMode();
        showToast("คลิกค้างแล้วลากเส้นจากโหนดหนึ่งไปอีกโหนดหนึ่ง", "info");
    }
    ...
    function checkMatch(inputElement, expectedValue, isPrev = false, isSet = false) {
        ...
        if (isSet) {
            // Split and normalize commas/spaces for set matching
